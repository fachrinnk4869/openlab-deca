...
def main(args):
    ...
    # load test images
    testdata = datasets.TestData(args.inputpath, iscrop=args.iscrop,
    ...
    for i in tqdm(range(len(testdata))):
        ...
        with torch.no_grad():
            ...
            if args.render_orig:
                ...
                original_image = testdata[i]['original_image'][None, ...].to(
                    device)
        ...
        if args.saveVis:
            ...
            if args.render_orig:
                cv2.imwrite(os.path.join(
                    savefolder, name + '_vis_original_size.jpg'), deca.visualize(orig_visdict))
        if args.saveImages:
            for vis_name in ['inputs', 'rendered_images', 'albedo_images', 'shape_images', 'shape_detail_images', 'landmarks2d']:
                ...
                if args.render_orig:
                    ...
                    cv2.imwrite(os.path.join(savefolder, name, 'orig_' + name + '_' +
                                vis_name + '.jpg'), util.tensor2image(orig_visdict[vis_name][0]))
